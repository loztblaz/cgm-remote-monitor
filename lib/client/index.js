...
client.load = function load(serverSettings, callback) {
  ...
  client.bottomOfPills = function bottomOfPills ( ) {
    ...
    var toolbarCalStatus = bottomOfStatusPills
  };
    ...
    if (status !== 'current') {
      ...
      if (currentMgdl < 39) {
        ...
        if (delta) {
          ...
          title = s(scaleBg(currentMgdl)) + s(deltaDisplay) + s(client.direction.info(client.latestSGV).label) + title + toolbarCalStatus;
		console.log(toolbarCalStatus)
        }
      }
    }
    return title;
